...
cut_axial=1536;
downsample_rate=6;
data_name = 'RF_DS_A0_RBL_05'; %P99, core0, 256x140
data_name_mask='masked_DS_A0_RBL_05';
path_data = 'Z:\shared\images\ProstateVGH-2\Data\Patient99\2019-11-20-11-43\BMode\ROI_Data\Down_Sample\';
...
path_mask = 'Z:\shared\images\ProstateVGH-2\Data\Patient99\2019-11-20-11-43\BMode\ROI_Data\Down_Sample\';
...
% RF_dims=[2344,282];%p91 c0
RF_dims=[2544,392]; %P99 core0
if RF_dims(2)==282 %reach 280
           left_c=2;right_c=1;
elseif RF_dims(2)==392 %reach 280
           left_c=57;right_c=56;
elseif RF_dims(2)==372 %reach 280
           left_c=47;right_c=46;
elseif RF_dims(2)==414 %reach 280
           left_c=68;right_c=67;
elseif RF_dims(2)==438 %reach 420
           left_c=10;right_c=9; DS_L=3;
elseif RF_dims(2)==496 %reach 420
           left_c=39;right_c=38; DS_L=3;
elseif RF_dims(2)==466 %reach 420
           left_c=24;right_c=23; DS_L=3;
elseif RF_dims(2)==530 %reach 420
           left_c=56;right_c=55; DS_L=3;
%            left_c=150;right_c=99;
elseif RF_dims(2)==560 % reach 420
           left_c=71;right_c=70; DS_L=3;
end  

%% load predicitons
load([path_pred,'_pred.mat']);  %prediction:(105, 256, 128, 2)
pred=squeeze(prediction(53,:,:,2)); % core 0 p99
%% load Masek RF_DS
mask_RF=imread([path_mask,'masked_DS_A0_RBL_05.jpg']);  %prediction:(105, 256, 128, 2)
figure
imagesc(mask_RF)
...
%Convert to Bmode
    ...
    RF_depth         = 0.065;     % RF/B-mode depth (end - start) (m) ?
    %%
...
Envelope = abs(hilbert(RF_data_sample)).^.3; %Envelop detection for B-mode                     
    ...
    nPhi             = 1;%RF_size(3); %1
    dX               = 0.011813335120677948*0.01;%RF_depth/RF_size(1);%axial resolution measured in machine: 0.1e-3;%Dicom: 0.009e-3;%reference 0.6e-5;                                             % 3D voxel size
...
img=imread('Z:\shared\images\ProstateVGH-2\Data\Patient99\2019-11-20-11-43\BMode\ROI_Data\TestImages\BMROI_test_0.bmp');

J(J<0.6*max(J(:)))=0;
figure;imagesc(J)
img(1:615,:,1)=J/max(J(:))*255;
figure;imagesc(img)