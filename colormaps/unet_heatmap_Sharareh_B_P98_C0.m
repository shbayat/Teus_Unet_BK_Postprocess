...
% data_name = 'RF_DS_A0_RBL_04'; %P98, core0
% data_name_mask='masked_DS_A0_RBL_04';
data_name = 'RF_DS_A0_RBL_04'; %P98, core0
data_name_mask='masked_DS_A0_RBL_04';
path_data = strcat(data_dir,'\BMode\ROI_Data\Down_Sample\');
...
path_mask = strcat(data_dir,'\BMode\ROI_Data\Down_Sample\');
S = dir(strcat(path_mask,'masked_DS_A*.jpg'));
...
for ii=1:2
    pred=squeeze(prediction(43,:,:,ii)); % p98,core0
    %% load Masek RF_DS
    mask_RF=imread([path_mask,S(CoreID+1).name]);  %prediction:(105, 256, 128, 2)
    figure
    imagesc(mask_RF)
