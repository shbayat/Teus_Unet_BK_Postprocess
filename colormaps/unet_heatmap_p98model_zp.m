%% 
%% this version used the same bmode without needle - v2 of heat map
...
ind=find(idcore_test==984); %  p98, core11       
cut_axial=1536;
% downsample_rate=6;
Patient=98;
CoreID=11; %
...
RF_dims=[2344,414]; %P91 core8-4+4
...
data_dir1=strcat('z:\\shared\images\ProstateVGH-2\Data\Patient',num2str(Patient));
dirlist=dir(data_dir1);
dir3 = strcat(data_dir1,'\',dirlist(3).name,'\');
BM_file='Bmode_A11_LAM_385.dat';
save_dir=strcat(dir3,'BMode\ROI_Data\WholeProstate\');

w = 756;
h =616;


fid = fopen(strcat(dir3,BM_file));
...
img=squeeze(rotframes(:,:,end));
img2 =uint8( cat(3, img,img,img));
h=imagesc(img);colormap('gray');
...
%     pred = [pred, zeros(256,12)];
    ...
    if ii==1
        img2(:,:,3)=double(img2(:,:,3))+floor(J/max(J(:))*125).*BM_mask;
    else
        img2(:,:,1)=double(img2(:,:,1))+floor(J/max(J(:))*125).*BM_mask;
    end
    
end
    figure;imagesc(uint8(img2))
    c=imfuse(BM_mask,img);
    figure;imagesc(c)
