...
% ind=find(idcore_test==1096); %  p110, core8 
ind=find(idcore_test==908); %  p91, core10   
% ind=find(idcore_test==973);
cut_axial=1536;
% downsample_rate=6;
Patient=91;
CoreID=9; %
...
path_pred='\\smbhome\rcl\shared\images\ProstateVGH-2\Data\Dataset\InProstate\Zero-pad\Best_Models\Not_Normalized\2020_10_05_09_26_59_asd\'; %zeropad model
% path_pred='F:\PhD\2020-03-24-Uncertainty\MATLAB\mcdropout\unet\'; %MC dropout predictions path
...
RF_dims=[2344,282]; %P91 core8-4+4
% RF_dims=[2344,414]; %P98 core0-B

%% load predicitons
load([path_pred,'_pred.mat']);  %prediction:(105, 256, 528, 2)

%% MC dropout loading
% load([path_pred,'predictions\mc_predictions_unet_d40_zp.mat']);  %prediction:(50, 105, 256, 128, 2)
% prediction = squeeze(mean(pred,1));
% % prediction = squeeze(std(pred,1));
% % prediction = squeeze(var(pred,1));

%%
img=imread([data_dir, '\BMode\ROI_Data\TestImages\BMROI_test_', int2str(CoreID),'.bmp']);
% img=imread([data_dir, '\BMode\ROI_Data\TestImages\BMROI_test_7.bmp']);
...
if CoreID==0
    ...
    if CoreID>=10
    BM_mask=imread([data_dir, '\BMode\ROI_Data\WholeProstate\', SS(CoreID-8).name]);
    SS(CoreID-8).name
    else
    BM_mask=imread([data_dir, '\BMode\ROI_Data\WholeProstate\', SS(CoreID+3).name]);
    SS(CoreID+3).name
%     BM_mask=imread([data_dir,'BMode\ROI_Data\WholeProstate\BM_mask_A7_LBM_258.bmp']);
    end
end

figure(1);subplot(1,2,1); imagesc(img);title("91-C7")
subplot(1,2,2); imagesc(BM_mask);title("mask-Bmode-P91-C7")
%%
new_softmax = zeros(616,756,2);
for ii=1:2
...
figure; imagesc(squeeze(new_softmax(:,:,1)))

new_softmax = reshape(new_softmax,[616*756,2]);
new_softmax = softmax(new_softmax');
new_softmax = reshape(new_softmax',[616,756,2]);
J= new_softmax;
...
infu = zeros(size(img));
img(:,:,3)=double(img(:,:,3))+floor(J(:,:,1)*75).*BM_mask;
img(:,:,1)=double(img(:,:,1))+floor(J(:,:,2)*75).*BM_mask;

% infu(:,:,3) = floor(J(:,:,1)*255).*BM_mask;
% infu(:,:,1) = floor(J(:,:,2)*255).*BM_mask;
% overlay = imfuse(img, infu);

    
figure;
h = imagesc(img); 
% saveas(h, 'unet_plot\colormap\P91C11_m.png')  
